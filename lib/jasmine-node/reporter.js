...
printRunnerResults = function(runner){
  ...
  msg += results.passedCount + ' pass' + ((results.passedCount === 1) ? '' : 'es') + '\n';
  return msg;
};
  ...
  reportSuiteResults: function(suite) {
    ...
    specResults.items_.forEach(function(spec){
      if (spec.description && spec.failedCount > 0) {
        ...
        outerThis.log_.push(outerThis.stringWithColor_('  it '  + spec.description, outerThis.color_.fail()));
        ...
        if (outerThis.isVerbose_) {
          outerThis.log_.push(outerThis.stringWithColor_('  it '  + spec.description, outerThis.color_.pass()));
        }
      }
    });
		if(this.isVerbose_)
