...
function watchDirAndReRun(dirs){
  var diffInSecs = (+new Date() - lastChecked) / 1000;
  var cmd = 'find ' + dirs.join(' ') + ' -type f -mtime -' + (~~diffInSecs) + 's';
  exec(cmd, function(error, stdout) {
    if (!error){
      lastChecked = +new Date();
      if (stdout){
        console.log('files changed, rerunning tests  ', stdout.split('\n').join('\n  '));
...
executeSpecsInFolder(function(){
  if (watchDir){
    var dirs = watchDir.split(',').map(function(d){ return path.join(process.cwd(), d) });
    watchDirAndReRun(dirs);
  }
});
